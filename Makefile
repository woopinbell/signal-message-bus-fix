...
FAULT_CFLAGS := $(CFLAGS) -DMT_WRITE_CALL=mt_test_write -DMT_EVENT_WRITE=mt_test_event_write -include tests/write_fault.h
RM := rm -rf
OBJ_DIR := obj
...
fclean: clean
	$(RM) $(NAME_SERVER) $(NAME_CLIENT) $(NAME_SESSION_SENDER) \
		...
		$(NAME_STALE_EXEC) $(NAME_STALE_SERVER_EXEC)
